...
step (t,dir) {pod,planets} = {pod=pod,planets=planets}
              
render : (Int,Int) -> Game -> Element
render (w',h') {pod,planets} =
  let (w,h) = (toFloat w', toFloat h')
  in collage w' h'
    ((circle 10 |> outlined (solid (rgb 50 150 200)) |> move (V2.getX pod.pos, V2.getY pod.pos))
    :: map (\planet -> circle planet.mass |> outlined (solid (rgb 200 150 50)) |> move (V2.getX planet.pos, V2.getY planet.pos)) planets)
